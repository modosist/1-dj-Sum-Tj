...
void runOne(string instancePath, string solutionPath, long time_limit);
...
int runProblem(long time_limit) {
    cout << ">>start solving\n";
    ...
    auto res = pb.solve_init(ttRes, short(Config::K), stop_time);
    if (pb.N < 0) {
...
void runOne(string instancePath, string outputPath, long time_limit) {
    logfile.open("log.txt", ios::out);
    ifstream file(instancePath);
    if (!file) {
        cout << instancePath << " not found. Exiting..." << endl;
        return;
    }
...
!Config::SOLVE_ONE && Config::ID_START > 1) {
            ...
            for (int indId = 0; indId < Config::ID_START - 1; indId++) {
                ...
                if (fres.fail()) {
                    cerr << "Read res file failed at id = " << indId + 1
        ...
        for (double R = 0.2; R <= 1.0; R += 0.2) {
            ...
            for (double T = 0.2; T <= 0.8; T += 0.2) {
                ...
                for (int j = 0; j < nbInsPerRT; j++)// For each instance
                {
                    ...
                    ss.str("");
                    ss << dataFolder << sIns << "/SDT_" << sIns << "_" << setprecision(1) << fixed << R << "_" << T
                       << "_" << (j + 1) << fileSuffix;
                    ifstream file(ss.str());
                    if (!file) {
                        cout << ss.str() << " not found. Exiting..." << endl;
                        return 0;
                    }
                    // Ignore comments
                    while (file.peek() == '#')file.ignore(1000, '\n');
...
#else
                    if (Config::SOLVE_ONE || sIns > Config::SIZE_START ||
                        (sIns == Config::SIZE_START && indId + 1 >= Config::ID_START))
#endif
                    {
                        /////////////////////////
                        // Solving...
                        ...
                        if (Config::USE_SLAVE) {
                            ...
                            string currDir = (pos == Config::PROG_NAME.npos) ? "" : Config::PROG_NAME.substr(0,
                                                                                                             pos + 1);
                            ...
                            ss.str("");
                            ss << currDir << Config::EXE << " " << Config::CONFIG_FILE << " " << sizeIns << " "
                               << indId + 1;
                            //cout << ss.str() << endl;
                            system(ss.str().c_str());
                        ...
                        //!reuse the memory
                        if (resolve) {
                            cout << "Processing the memory for resolving...Current nbentry=" << Problem::pbMemo.nbentry
                                 << endl;
                        ...
                        result[indId].szmerL = Stat::ctrMemRemove == 0 ? 0 : (Stat::szMemRemove /
                                                                              Stat::ctrMemRemove);// Problem::ctrMerL == 0 ? 0 : Problem::ctrSzMerL / Problem::ctrMerL;
                        result[indId].nbmovR = Problem::ctrMovR;
                        result[indId].nbmerR = Stat::ctrCutDb2;// Problem::ctrMerR;
                        result[indId].szmerR = Stat::ctrCutDb2 == 0 ? 0 : Stat::szCutDb2 /
                                                                          Stat::ctrCutDb2;// Problem::ctrMerR == 0 ? 0 : Problem::ctrSzMerR / Problem::ctrMerR;
...
void makeStat(string filename, Result *result, int nbIns, int nbInsPerRT) {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        for (int j = 0; j < nbInsPerRT; j++)// For each instance
        {
            ...
            szMerPerRT += (result[indId].szmerL * result[indId].nbmerL + result[indId].szmerR *
                                                                         result[indId].nbmerR);// / (result[indId].nbmerL + result[indId].nbmerR);
